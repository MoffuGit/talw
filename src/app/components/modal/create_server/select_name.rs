...
pub fn SelectName() -> impl IntoView {
    ...
    let image_preview_url = RwSignal::<Option<String>>::new(None);
    ...
    view! {
        <form
            on:submit=move |ev: SubmitEvent| {
                ...
                create_server.dispatch_local(form_data);
            }
            on:reset=move |_| {
        ...
        >
            <div class="px-6 pt-6 relative flex flex-col justify-start items-center ">
                <div class="text-center font-bold text-2xl leading-[30px]">
                    Customize your server
                </div>
                <div class="mt-2 text-center text-base leading-5 font-normal">
            ...
            <div class="px-2 my-4">
                <div class="pt-1 flex justify-center">
                    <div class="w-20 h-20 flex-justify-center relative">
                        <input
                            ...
                            name="image"
                            accept="image/*"
                            class="absolute top-0 left-0 w-full h-20 opacity-0 z-50"
                            ...
                            on:change=move |_| {
                                if let Some(node) = image_input_ref.get() {
                                    if let Some(file) = node.files().and_then(|files| files.item(0))
                                    {
                                        image_preview_url
                                            .set(
                                                Url::create_object_url_with_blob(&Blob::from(file)).ok(),
                                            )
                                    }
                                }
                            }
                        ...
                        {move || match image_preview_url.get() {
                            ...
                            Some(url) => {
                                ...
                                view! {
                                    <img
                                        class="w-20 h-20 object-cover absolute top-0 left-0 z-40 rounded-full"
                                        src=url.get_value()
                                        on:load=move |_| {
                ...
                <div class="mt-6">
                    ...
                    <div class="flex flex-col">
                        <Transition fallback=move || ()>
                            <input
                                name="name"
                                class="input input-secondary font-medium p-[10px] h-10 text-base w-full rounded-[3px]"
                                type="text"
                                placeholder=user
                            />
                        </Transition>
                ...
                <div class="mt-2 pb-1 text-xs leading-4 font-normal">
            ...
            <div class="relative p-4 overflow-x-auto flex justify-between items-center bg-base-300/50">
                ...
                <button
                    ...
                    disabled=move || create_server.pending().get()
                >
                    Create Server
                </button>
            </div>
