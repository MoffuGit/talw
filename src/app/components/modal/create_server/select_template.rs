...
pub fn SelectTemplate() -> impl IntoView {
    view! {
        ...
        <div class="overflow-x-hidden overflow-y-scroll mt-6 h-[330px] px-4 pb-2 relative">
            <Templates template=ServerTemplate::Default>{ServerTemplate::Default.to_string()}</Templates>
            <div class="mt-[12px] mb-[8px] uppercase font-bold text-[12px]">"start from a template"</div>
            {ServerTemplate::iter().filter(|&template| template != ServerTemplate::Default).map(|template| view!{<Templates template=template>{template.to_string()}</Templates>}).collect_view()}
        </div>
...
fn Templates(template: ServerTemplate, children: Children) -> impl IntoView {
    ...
    view! {
        <SlideForward value=2 class="rounded-lg border-secondary border-[1px] border-solid mb-2  w-full p-0">
            <div class="flex items-center" on:click=move |_| selected_template.update(|value| *value = template)>
