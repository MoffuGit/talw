...
pub fn SelectTemplate() -> impl IntoView {
    view! {
        ...
        <div class="overflow-x-hidden overflow-y-scroll mt-6 h-[330px] px-4 pb-2 relative">
            {ServerTemplate::iter().map(|template| view!{<Templates template=template>{template.to_string()}</Templates>}).collect_view()}
        </div>
        <div class="relative p-4 overflow-x-auto flex-col items-start bg-base-300">
