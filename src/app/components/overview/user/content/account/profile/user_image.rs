...
pub fn UserImage() -> impl IntoView {
    ...
    view! {
        ...
        node_ref=form_ref
        >
            <input type="file" name="user_image" accept="image/*" class="w-36 h-36 absolute top-16 left-6 rounded-full opacity-0 z-[60]" on:change=move |evt| {
            ...
            } />
            {
                move || match image_preview_url.get() {
                    Some(url) => {
                        ...
                        view! {
                            <img class="w-36 h-36 object-cover absolute top-16 left-6 rounded-full border-8 border-base-200 z-50" src=url.get_value()
                                on:load=move |_| {
                    ...
                    None => view! {
                        <div class="w-36 h-36 absolute top-16 left-6 rounded-full border-8 bg-base-100 border-base-200 z-50"/>
                    }.into_view()
                }
            }
            <div class="w-36 h-36 absolute top-16 left-6 rounded-full transition-opacity opacity-0 hover:opacity-100 bg-base-100/30 z-[60] flex items-center justify-center" >
