...
pub fn ContextServerMenu(
    ...
    view! {
        <ContextMenuProvider open=open modal=false hidden=hidden>
            ...
            <ContextMenuContent ignore=vec![leave_server_node, create_channel_node, create_category_node, invite_people_node] class="transition-all ease-out w-[188px] flex flex-col h-auto py-[6px] px-2 bg-[#dfdfe2] dark:bg-[#0d0d0d] z-40 rounded".to_string()>
                <InvitePeopleModal content_ref=invite_people_node invite_code=invite_code class="flex justify-between hover:bg-primary items-center w-full text-sm py-[6px] px-2 my-0.5 group rounded" on_click=on_click_item>
                        ...
                        move || {
                            if let Some(Ok(true)) = member_can_update.get() {
                                view! {
                                    ...
                                    <ServerMenuAdminItems nodes=ServerMenuNodes {
                                        create_channel_node,
                                        create_category_node,
                                    } server=server.get_value() on_click=on_click_item/>
                                }.into_view()
                            } else {
                                ().into_view()
                            }
...

                    {
                        move || {
                            if let Some(Ok(false)) = member_can_update.get() {
                                view! {
                                    <div class="divider relative my-0 mx-1 w-auto"/>
                                    <ServerMenuGuestItems leave_server_node=leave_server_node server=server.get_value() on_click=on_click_item/>
                                }.into_view()
                            } else {
                                ().into_view()
                            }
