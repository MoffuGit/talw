...
pub fn LeaveThread(thread_id: Uuid, server_id: Uuid, class: &'static str) -> impl IntoView {
    view! {
        <div on:click=move |_| use_thread().leave_thread.dispatch(LeaveThread { thread_id, server_id }) class=class>"Leave Thread"</div>
    }
}
...
pub fn JoinThread(thread_id: Uuid, server_id: Uuid, class: &'static str) -> impl IntoView {
    view! {
        <div on:click=move |_| use_thread().join_thread.dispatch(JoinThread { thread_id, server_id }) class=class>"Join Thread"</div>
    }
}
