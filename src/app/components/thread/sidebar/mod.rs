    ...
    view! {
        {
            move || {
                match current_thread() {
                    Some(Ok(thread_id)) => {
                        let current_thread = create_resource(move || (use_thread().delete_thread.version().get()), move |_| get_thread(thread_id, channel_id().into_uuid()));
                        view!{
                    ...
                    Some(Err(_)) | None => view!{<Redirect path=format!("/servers/{}/{}", server_id, channel_id())/>}.into_view(),
                }
            }
