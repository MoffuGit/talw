...
pub fn UserSettingsSideBar() -> impl IntoView {
    view! {
        <div class="w-2/5 min-w-[218px] h-full flex bg-base-200 justify-end pt-8 pr-1">
...
pub fn UserSettingsSelect(select: UserSettings) -> impl IntoView {
    let settings = use_context::<UserOverviewContext>()
        ...
        .settings;
    view! {
        <div class="w-48 h-8 rounded pl-3 mb-0.5 flex items-center hover:bg-base-content/10 text-base-content/70" on:click=move |_| settings.set(select)>
            {select.to_string()}
        </div>
    }
}
